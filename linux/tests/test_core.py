...
class UpdateTests(unittest.TestCase):
    def offer(self):return dict(draft=False,prerelease=False,assets=[dict(name="SnoopyLinuxUpdate-1.1.1.tar.gz",state="uploaded",size=100,digest="sha256:"+"a"*64,url=updater.API+"/releases/assets/1")])
    def test_select(self):
        self.assertEqual(updater.select([self.offer()])["version"],"1.1.1")
        self.assertIsNone(updater.select([self.offer()],"1.1.1"))
